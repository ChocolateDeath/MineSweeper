...
//		System.out.println("x = " + x + " y = "+y);
	...
	*/
	public void removeAdjacentButtons(Graphics g,int r, int c)
	{
		 ...
		 if (r < leftedge || r > button.length || c < leftedge ||c > button.length) 
			 		return; // check if within edges
		 //no mines around
         if ( button[r][c].getNoOfMinesAround() == 0 && !button[r][c].isShowing()) {
             button[r][c].showIt(g);
             //Check the 8 boxes clockwise around the button recursively
        	 ...
        	 button[r][c].showIt(g);//just reveal button
      }
